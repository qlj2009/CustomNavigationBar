...
#define TABLE_CELL_SELECTED_BACKGROUND  {0, 1, 0, 1, 0, 0.866, 0, 1}
...
@synthesize position,isSelectedView;


+(UACellBackgroundView *)backgroundView{
  UACellBackgroundView *v = [[UACellBackgroundView alloc] initWithFrame:CGRectZero];
  return [v autorelease];
}

+(UACellBackgroundView *)backgroundViewIsSelectedView:(BOOL)isSeleced{
  UACellBackgroundView *v = [[UACellBackgroundView alloc] initWithFrame:CGRectZero];
...
-(void)drawRect:(CGRect)aRect {
  ...
  for(int i=0;i<8;i++) 
  {
    components[i]= isSelectedView?csm[i]:cm[i]; 
  }
